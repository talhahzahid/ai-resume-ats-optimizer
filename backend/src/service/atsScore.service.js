...
// ===== Domain-wise keyword banks =====
const KEYWORD_BANKS = {
  webDev: [
    "javascript", "react", "node", "api", "sql", "database",
    "rest", "typescript", "aws", "git", "agile",
    "authentication", "testing", "docker", "llm",
  ],
  javaBackend: [
    "java", "spring", "spring boot", "hibernate", "maven",
    "microservices", "rest api", "sql", "jpa", "junit",
    "docker", "kubernetes", "git", "agile", "kafka",
  ],
  python: [
    "python", "django", "flask", "fastapi", "pandas",
    "numpy", "sql", "rest api", "docker", "git",
    "machine learning", "pytest", "celery", "postgresql", "aws",
  ],
  dataScience: [
    "python", "sql", "machine learning", "tensorflow", "pytorch",
    "pandas", "numpy", "scikit-learn", "data visualization",
    "statistics", "nlp", "deep learning", "aws", "spark", "etl",
  ],
};
...
const detectDomain = (resumeText) => {
  ...
  for (const [domain, keywords] of Object.entries(KEYWORD_BANKS)) {
    const matches = keywords.filter((k) => resumeText.includes(k)).length;
...
export const calculateATSScore = (resume) => {
  ...
  const rawBreakdown = {
    ...
    skills: checkSkills(resume),
    experience: checkExperience(resume),
    keywords: checkKeywords(resume, domain),
    education: checkEducation(resume),
    achievements: checkAchievements(resume),
    formatting: checkFormatting(resume),
  };
